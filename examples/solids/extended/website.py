...
from g4f.image.copy_images import secure_filename
...
class Website:
    ...
    def _index(self, filename = "index"):
        path = os.path.abspath(os.path.join(DIST_DIR, f"index.html"))
        # return f"{path}:{os.path.exists(path)}"
        # Menggunakan send_from_directory untuk melayani file statis dengan benar
    ...
    def _assets(self, name: str):
        return send_from_directory(os.path.abspath(DIST_DIR), f"assets/{name}")
    def _js(self, name: str):
        return send_from_directory(os.path.abspath(DIST_DIR), f"js/{name}")
    def _css(self, name: str):
        return send_from_directory(os.path.abspath(DIST_DIR), f"css/{name}")
    def _nm(self, name: str):
        return send_from_directory(os.path.abspath(DIST_DIR), f"node_modules/{name}")
    def _gh(self, name):
