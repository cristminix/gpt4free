...
async def example_usage():
    ...
    """
    # Konfigurasi dasar
    model = "GLM-4-6-API-V1"  # Ganti dengan model yang tersedia
    messages = [
    ...
    # args = await ExtendedGLM.get_args_from_nodriver(ExtendedGLM.url)
    ...
    async with StreamSession(**args) as session:
        try:
            # Menggunakan ExtendedGLM untuk membuat generator
            async for chunk in ExtendedGLM.create_async_generator(
                model=model,
                messages=messages,
            ...
            print(f"{e}")
            # print(f"Error occurred (this is expected if API requires authentication): {e}")
            # print("The ExtendedGLM class is properly implemented and imported.")
