...
from examples.custom_inference_api.LMARENABETA_MODEL_INFO import get_lmarenabeta_models
from examples.custom_inference_api.PUTERJS_MODEL_INFO import get_putterjs_models
...
from g4f.providers.any_provider import PROVIERS_LIST_1, PROVIERS_LIST_2, PROVIERS_LIST_3, AnyProvider
...
DEFAULT_PORT = 1337
DEFAULT_TIMEOUT = 600
logger.debug(f"Using default port: {DEFAULT_PORT}, default timeout: {DEFAULT_TIMEOUT}")
@asynccontextmanager
...
def get_name_and_provider(name_provider: str) -> Tuple[str, str]:
    ...
    if ':' not in name_provider:
        return  tuple("qwen-2.5:HuggingSpace".split(':', 1))
...
def get_model_by_providers():
    ...
    for model in models:
        ...
        for provider_name in model["providers"]:
            if provider_name not in AppConfig.ignored_providers:
    ...
    return model_by_providers + get_lmarenabeta_models()
def create_app():
...
class AppConfig:
    ignored_providers: Optional[list[str]] = ["LegacyLMArena","Grok","OpenaiChat","PuterJS"]
...
class Api:
    ...
    def register_authorization(self):
        if AppConfig.g4f_api_key:
            debug(f"Register authentication key: {''.join(['*' for _ in range(len(AppConfig.g4f_api_key))])}")
        if has_crypto:
