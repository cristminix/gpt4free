from sqlalchemy import Column, Integer, String, Text, DateTime, func, select, and_
from sqlalchemy.dialects.sqlite import DATETIME
from datetime import datetime
from typing import Optional, List, Dict, Any
import asyncio

# Import dari db_connection.py
from .models import UsageModel,Usage, base_db
...
async def insert_usage(
    ...
    # Menggunakan db dari Flask-SQLAlchemy
    with base_db.app.app_context():
        try:
            ...
            base_db.session.add(db_usage)
            base_db.session.commit()
            base_db.session.refresh(db_usage)
...
async def update_usage(
    ...
    with base_db.app.app_context():
        try:
            ...
            db_usage = base_db.session.query(UsageModel).filter(UsageModel.id == id).first()
...
async def get_usage_by_provider_model_date(
    ...
    with base_db.app.app_context():
        try:
            ...
            if ipaddr:
                db_usage = base_db.session.query(UsageModel).filter(
                    and_(
...
async def get_all_usages() -> List[Usage]:
    ...
    with base_db.app.app_context():
        try:
            db_usages = base_db.session.query(UsageModel).all()
