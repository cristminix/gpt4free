...
class ExtendedGLM(GLM):
    ...
    @classmethod
    def create_signature_with_timestamp_new(cls, input_string: str, data_string: str, timestamp):
        import time
        import hashlib
        import hmac
        import base64
        
        timestamp_number = int(timestamp) if timestamp else int(time.time() * 1000)
        timestamp_string = str(timestamp_number)
        
        # Encode data_string to base64
        encoded_data = data_string.encode('utf-8')
        base64_encoded_data = base64.b64encode(encoded_data).decode('utf-8')
        
        signature_data = f"{input_string}|{base64_encoded_data}|{timestamp_string}"
        time_slot = timestamp_number // (5 * 60 * 1000)  # 5 minutes in milliseconds
        
        # Create base signature using hmac
        base_signature = hmac.new(
            "junjie".encode("utf-8"),
            str(time_slot).encode("utf-8"),
            hashlib.sha256
        ).hexdigest()
    ...
    def get_endpoint_signature(cls, token: str, user_id: str, user_prompt: str):
        ...
        a_tstamp = auth_params["timestamp"]
        
        # debug.log(f"Prompt:{user_prompt}")
        last_user_prompt = user_prompt.strip()
        
        # Create signature with timestamp
        signature_data = cls.create_signature_with_timestamp_new(sorted_payload, last_user_prompt,a_tstamp)
    ...
    def get_auth_from_cache(cls):
        ...
        if os.path.exists(cache_file_path):
            ...
            # Check if the file is less than 5 minutes old (5 * 60 seconds)
            if time_diff < 5 * 60:
                try:
